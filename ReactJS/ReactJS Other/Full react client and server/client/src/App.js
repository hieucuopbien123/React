...
class App extends Component {
    ...
    .then(res => {
      ...
      this.setState(prevState => ({
        news: prevState.news.map(
          ...
          //news là mảng chứa các object, elm là các object, ...elm sẽ gán các thuộc tính của objet cũ cho mới, các thuộc 
          //tình còn lại là Name, Age sẽ lấy khác
        ),
        modalIsOpen: false
      }))
    })
  ...
  //tức là nếu app.get nhiều lần thì nó chỉ bắt đúng cái trùng đối số thôi.
    axios.get('/api/new')
          .then(res => {
            ...
            //lúc này dataArray đang là 1 object {new: mảng các object}
            this.setState({
              news: dataArray.new
            })
          })
  ...
  handleInsertSubmit = (event) => {
    ...
    //chú ý thuộc tính trong sql của ta có viết hoa thì phải đúng
    //truyền vào đối số thứ 2 của post luôn là object, nên phải convert hết sang object. Kể cả array cx phải convert sang
    //object. Các hàm ở thư viện ngoài thg dùng quy tắc chung là dùng object với mọi loại cho tiện như v
    axios.post('/api/insert', newItem, { headers: {'Content-Type': 'application/json'} })
      .then(res => {
        ...
        //tính khác, ví dụ headers Content-Type tùy TH nếu k có sẽ lỗi. Là object thì phải xđ là type application/json
  ...
  //Cơ chế và cách làm chuẩn hơn: cách làm hiện tại là k chuẩn=> ở lần đầu tiên loading với cái file hiện tại thì server nó sẽ làm như sau: đầu tiên nó chạy lệnh query trước
  //để lấy dữ liệu từ data base và gặp hàm get mặc định ta set hàm get này sẽ được lưu lại và khi nào có client request thì sẽ thực hiện hàm get này. Tuy nhiên đáng lẽ đặt
  //query trong hàm get thì ta lại đặt hàm get trong query nên dữ liệu của hàm get là fix luôn do ta get như v. Các hàm get về sau nếu lấy cùng dữ liệu với các hàm get trc thì
  //nó vẫn làm theo thứ tự làm cho hàm sau đè lên hàm trc. Nhưng ở đây ví dụ ta vào trang web và lấy đc dữ liệu->vào sql đổi dữ liệu->refresh lại trang web->tự động thực hiện
  //lại hàm get nhưng nó éo gọi query vì dữ liệu hàm get bị fix k thực hiện lại nên k có sự thay đổi-> đó là 1 cái sai. Sau đó ta thêm dữ liệu vào -> submit -> axios gọi post
  //server nhận được thực hiện query insert nhét vào cơ sở dữ liệu thực. Đông thời lấy dư liệu lại lần nx và gửi lại cho client. client nhận đc thực hiện then sẽ cập nhập lại
  //state-> in ra màn hình dữ liệu với state mới. Refresh lại trang phát nx-> gọi lại hàm get cũ với dữ liệu fix lưu vào biến test-> nhưng nó k hiện lại dữ liệu cũ vì trong
  //quá trình post ta đã gán lại dữ liệu mới cho test thành ra test đc cập nhập-> hàm get cũ in ra test nên refresh vẫn ra y nguyên như v
